...
| **シェイプ** | 10 | 図形/テキストボックス/画像/線の追加（Z順序指定つき、テキストの下に置くことも可）、一覧、情報取得（位置・塗り・線・テキストフレーム）、更新（単体/複数/スライド全体、絶対値・相対値）、削除、Z順序 |
| **テキスト** | 10 | テキスト設定（全体/書式を保ったまま一部/ラン単位）、取得（寸法とはみ出し判定つき）、書式設定（単一スパン/一括）、段落書式、箇条書き、検索置換（グループ内も可）、テキストフレーム、全テキストMarkdown抽出、組版チェック |
| **プレースホルダー** | 6 | 一覧、情報取得、テキスト設定 |
| **書式設定** | 3 | 塗りつぶし、線、影（図形またはテキストに適用） |
...
| **編集操作** | 6 | 元に戻す、やり直し（操作対象のプレゼンに対して）、スライド間シェイプ/書式コピー |
| **レイアウト** | 7 | 整列、分散配置、スライドサイズ、背景、反転、シェイプ結合 |
| **視覚効果** | 3 | グロー、反射（図形またはテキストに適用）、ぼかし |
| **コメント** | 3 | 追加、一覧、削除 |
...
操作対象はサーバー全体で1つだけです。Claude Desktop のように、1つのサーバーにすべての会話を通すクライアントもあります。複数の会話で別々のファイルを同時に扱うときは、各呼び出しに `presentation` を渡してください。その呼び出しだけに効き、アクティブ化済みの操作対象は変わりません。フルパス、ファイル名、拡張子なしのファイル名のいずれでも指定できます。開いていないファイルを指定するとエラーになり、別のファイルに処理が流れることはありません。

```python
ppt_add_textbox(params={...}, presentation="report.pptx")
# 他の会話が何をアクティブ化していても report.pptx に追加される
```
