...
- **グラフ、SmartArt、フリーフォーム。** 辞書にクラスもコマンドもないので、
  スクリプトからは作ることも編集することもできません。すでに置いてあるものは
  ただの図形として動かしたり読んだり消したりできます
...
- こまめに保存してください。一度も保存していないプレゼンテーションが作業中に
  丸ごと消えた例があり、原因はまだ分かっていません
